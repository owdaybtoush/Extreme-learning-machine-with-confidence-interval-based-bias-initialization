function [TrainingTime, TestingTime, TrainingAccuracy, TestingAccuracy,rank_NCI]...
=NCI(train_data, test_data, Elm_Type, NumberofHiddenNeurons, ActivationFunction,nayai)
rng(0)
...
tempH=InputWeight*P;
 ...
 xT = InputWeight;
CIFcn1 = @(xT,pa)std(xT(:),'omitnan')/sqrt(sum(~isnan(xT(:)))) * tinv(abs([0,1]-(1-pa/100)/2),sum(~isnan(xT(:)))-1) + mean(xT(:),'omitnan'); 
CIT1 = CIFcn1(xT,pa);
 Bias=CIT1(2)+(CIT1(2)-CIT1(1))* rand(NumberofHiddenNeurons,1 );
%  Bias=0.01*Bias;
