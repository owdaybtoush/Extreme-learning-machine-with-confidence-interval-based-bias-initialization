...
=NULCI(train_data, test_data, Elm_Type, NumberofHiddenNeurons, ActivationFunction,nayai)
      rng(0)
...
%    
       ...
       Biasup = CI(1)+(CI(1)-1).* rand(down,1); 
      Biasdown = CI(2)+(CI(2)-0).* rand(up,1); 
 
              Bias=[Biasdown;Biasup];
        Bias = 1 ./ (1 + exp(-Bias));

%         xlswrite('Biasconfdanceup1.xls',Bias)
